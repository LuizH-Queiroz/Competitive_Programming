...
public:
    int numDecodings(string s) {
        ...
        vector<long long> numWays(s.size() + 2);

        numWays[numWays.size()-1] = 0;
        numWays[numWays.size()-2] = 1;
        for (int i = numWays.size()-3; i >= 0; i--)
        {
            if (s[i] == '0')
            {
                numWays[i] = 0;
            ...
            else if (s[i] == '1')
            {
                ...
                if ((i+1) < s.size())
                {
                    if (s[i+1] != '*') numWays[i] += numWays[i+2];
                    else numWays[i] += numWays[i+2] * 9;
                }
            }
            else if (s[i] == '2')
            {
                ...
                if ((i+1) < s.size())
                {
                    if (s[i+1] >= '0' && s[i+1] <= '6') numWays[i] += numWays[i+2];
                    else if (s[i+1] == '*') numWays[i] += numWays[i+2] * 6;
                }
            }
            else if (s[i] >= '3' && s[i] <= '9')
            {
