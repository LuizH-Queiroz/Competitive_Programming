...
You are given a 0-indexed integer array dist of size n, where dist[i] is the initial distance in kilometers
...
You have a weapon that, once fully charged, can eliminate a single monster. However, the weapon takes one
minute to charge. The weapon is fully charged at the very start.
...
public:
    int eliminateMaximum(vector<int>& dist, vector<int>& speed) {
        ...
        while(!pq.empty() && weaponTime < pq.top())
        {
            killed++;
            weaponTime++;
            pq.pop();
