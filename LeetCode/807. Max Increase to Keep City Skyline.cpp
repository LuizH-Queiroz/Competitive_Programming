...
The height of a 0-height building can also be increased. However, increasing the height of a building should not affect the city's skyline from
...
public:
    int maxIncreaseKeepingSkyline(vector<vector<int>>& grid) {
        vector<int> maiorNaLinha(grid.size(), 0);
        vector<int> maiorNaColuna(grid.size(), 0);

        for (int i = 0; i < grid.size(); i++)
        {
            for (int j = 0; j < grid[i].size(); j++)
            {
                maiorNaLinha[i] = max(maiorNaLinha[i], grid[i][j]);
                maiorNaColuna[j] = max(maiorNaColuna[j], grid[i][j]);
            }
        }
        ...
        for (int i = 0; i < grid.size(); i++)
            for (int j = 0; j < grid[i].size(); j++)
                counter += max(min(maiorNaLinha[i], maiorNaColuna[j])-grid[i][j], 0);
